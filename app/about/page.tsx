...
export default function AboutPage() {
  ...
  const values = [
    ...
    { title: "الجودة والريادة", desc: "نلتزم بأعلى المعايير الطبية العالمية لضمان دقة التشخيص وفعالية العلاج.", icon: "⭐" },
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800" dir="rtl">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-8 font-bold text-[#007A7C]">
      ...
      <section className="bg-gradient-to-l from-[#007A7C] to-[#0c2f25] text-white py-20 px-4 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black mt-2 leading-tight">
            عن مستشفى الريادة الدولي
          </h1>
          <p className="text-teal-100 mt-6 text-lg md:text-xl leading-relaxed max-w-2xl mx-auto">
            نحن صرح طبي متكامل في قلب مدينة ذمار، نجمع بين الكفاءات الطبية الاستثنائية والتكنولوجيا الحديثة لنرسم معياراً جديداً للرعاية الصحية والتجميلية في اليمن.
          </p>
        </div>
      </section>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 grid grid-cols-2 md:grid-cols-4 gap-6 divide-x divide-x-reverse divide-slate-100">
          {stats.map((stat, idx) => (
            <div key={idx} className="text-center px-4">
              ...
              <div className="text-3xl font-black text-[#007A7C]">{stat.number}</div>
      ...
      <section className="py-20 max-w-7xl mx-auto px-4">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="relative">
            <div className="absolute -inset-4 bg-teal-50 rounded-3xl -z-10 transform rotate-3"></div>
            <img 
              src="https://images.unsplash.com/photo-1581594693702-fbdc51b2763b?q=80&w=1000&auto=format&fit=crop" 
              alt="فريق مستشفى الريادة" 
          ...
          <div className="space-y-10">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span className="w-12 h-12 bg-teal-50 text-[#007A7C] rounded-2xl flex items-center justify-center text-2xl font-black">🎯</span>
                <h2 className="text-3xl font-black text-slate-900">رسالتنا</h2>
              </div>
              <p className="text-slate-600 leading-relaxed text-lg border-r-4 border-[#007A7C] pr-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {values.map((val, idx) => (
              <div key={idx} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 hover:-translate-y-2 transition duration-300 group">
                <div className="w-16 h-16 bg-teal-50 text-[#007A7C] rounded-2xl flex items-center justify-center text-3xl mb-6 group-hover:scale-110 transition">
                  {val.icon}
                </div>
